from keras.layers import Dense, Input, Conv2D, merge, MaxPooling2D, Reshape, RepeatVector, Flatten, Permute, LSTM, Bidirectional, Activation
...
def attention_network_2(data):
    ...
    lstm1_ = Bidirectional(LSTM(p["ld"], return_sequences=True), name="LSTM1")(lstm0_)

    y_ = (AttentionDecoder(p["da"], p["do"], name='attention_' + str(p['da']))(lstm1_))

    return Model(inputs=i_, outputs=y_)


def attention_network_3(data):
    """
    this function return a simple attention network with convolutional and LSTM layers
    It is a light network designed for the Hamelin word dataset
    :param data: element of the class Data defined in ./data/reader
    ...
    # Convolutions ##
    conv1_ = Conv2D(p["cc1"], (3, 3), padding="same", strides=p["kmp1"], activation=Activation('tanh'))(i_)
    conv2_ = Conv2D(p["cc2"], (3, 3), padding="same", strides=p["kmp2"], activation=Activation('tanh'))(conv1_)
    conv3_ = Conv2D(p["cc3"], (3, 3), padding="same", strides=p["kmp3"], activation=Activation('tanh'))(conv2_)
    conv4_ = Conv2D(p["cc4"], (3, 3), padding="same", strides=p["kmp4"], activation=Activation('tanh'))(conv3_)
...
if __name__ == "__main__":
    ...
    model = attention_network_3(data)
    model.summary()
